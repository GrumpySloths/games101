...
void rst::rasterizer::rasterize_triangle(
    ...
    auto x_minmax = std::minmax_element(
        v.begin(), v.end(),
        [](const Vector4f& a, const Vector4f& b) { return a.x() < b.x(); });

    auto y_minmax = std::minmax_element(
        v.begin(), v.end(),
        [](const Vector4f& a, const Vector4f& b) { return a.y() < b.y(); });

    for (int x = int(x_minmax.first->x()); x <= int(x_minmax.second->x());
         x++) {
        for (int y = int(y_minmax.first->y()); y <= int(y_minmax.second->y());
             y++) {
