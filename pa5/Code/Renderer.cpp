...
void Renderer::Render(const Scene &scene) {
    ...
    for (int j = 0; j < scene.height; ++j) {
        for (int i = 0; i < scene.width; ++i) {
            ...
            // 按照给定的dir的设置，这里的znear设为了-1
            float t = scale;
            float r = scale * imageAspectRatio;
            y = (j * 1. + 0.5 - scene.height * 1. / 2) /
                (scene.height * 1. / 2) * t;
            x = (i * 1. + 0.5 - scene.width * 1. / 2) / (scene.width * 1. / 2) *
                r;
            // 这里y*-1是因为(0,0)点位于图片的左上角
            Vector3f dir = normalize(Vector3f(
                x, y * -1, -1));  // Don't forget to normalize this direction!
