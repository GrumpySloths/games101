...
namespace CGL {
    ...
    void Application::init() {
        ...
        // Create two ropes
        ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 16,
                             config.mass, config.ks, {0});
        ropeVerlet = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 16,
                              config.mass, config.ks, {0});
    }
    ...
    void Application::render() {
        ...
        for (int i = 0; i < config.steps_per_frame; i++) {
            ropeEuler->simulateEuler(1 / config.steps_per_frame,
                                     config.gravity);
            ropeVerlet->simulateVerlet(1 / config.steps_per_frame,
                                       config.gravity);
        }
        // Rendering ropes
        Rope *rope;
